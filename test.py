...
model={'input_shape':(tasks[0].shape[1],),'optimizer':SGD(lr=0.001), 'loss':'categorical_crossentropy', 'metrics':['accuracy'],'layers':3, 'units':400,'dropout':0,'activation':'relu'}
ewc = EWCClassifier(fisher_n=0,ewc_lambda=0.1,singleheaded_classes=50,model=model)
rewc = RotatedEWCClassifier(fisher_n=0,ewc_lambda=0.1,model=model)
oewc = OnlineEWCClassifier(fisher_n=0,ewc_lambda=0.1,gamma=10,singleheaded_classes=50,model=model)
...
evaluator.train(epochs=15,verbose=1)
